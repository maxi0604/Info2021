...
namespace Info2021
{
    static class FirstLevel
    {
        public static Level GetLevel(Game1 game) {
            ...
            for(int i = 0; i < 30; i++) {
                tiles.Add(new Tile(new TileInfo(game.resourceAccessor, 2, 0), i, 15));
                
            }
             for(int i = 0; i < 30; i++) {
                tiles.Add(new Tile(new TileInfo(game.resourceAccessor, 0, 2), 30, i));
                
            }
            for(int i = 12; i < 15; i++) {
                Tile tile;
                StaticCollider staticCollider;
                (tile, staticCollider) = GroundHelper(new TileInfo(game.resourceAccessor, 17, 0), 12, i);
                tiles.Add(tile);
            ...
            (tile1, staticCollider1) = GroundHelper(new TileInfo(game.resourceAccessor, 17, 0), 14, 11);
